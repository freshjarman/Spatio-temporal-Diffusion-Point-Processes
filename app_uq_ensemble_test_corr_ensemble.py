...
def get_args():
    ...
    # 添加新参数：每个样本的ensemble重复次数
    parser.add_argument('--n_repeats', type=int, default=10, help='每个样本的ensemble prediction重复次数')
    # cpu核数
...
if __name__ == "__main__":
    setup_init(opt)
    setproctitle.setproctitle("RF-STPP-UQ-MultiEnsemble")
    ...
    print(f'TEST Set evaluation with Multi-Ensemble Variance Analysis (n_repeats={opt.n_repeats})!')

    # 存储所有样本的多次ensemble方差和预测误差
    all_temporal_var_between_ensembles = []
    all_temporal_mae = []
    all_spatial_var_between_ensembles = []
    all_spatial_mae = []
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(testloader, desc="Processing test batch"):
            ...
            # 存储每个样本多次ensemble预测的结果
            temporal_means = []  # 存储每次ensemble的平均预测 [n_repeats, batch_size]
            spatial_means = []  # [n_repeats, batch_size, dim]

            # 执行多次ensemble预测
            for repeat_idx in range(opt.n_repeats):
                # 单次ensemble预测
                sampled_temporal_all, sampled_spatial_all = ensemble_sample(Model, batch_size, enc_out_non_mask, opt.n_ensemble, opt.dim)
                ...
                ensemble_temporal_mean = sampled_temporal_stack.mean(dim=0).cpu()  # [bsz, 1]
                ensemble_spatial_mean = sampled_spatial_stack.mean(dim=0).cpu()  # [bsz, dim]

                # 保存当次ensemble的平均预测
                temporal_means.append(ensemble_temporal_mean.detach())
                spatial_means.append(ensemble_spatial_mean.detach())

            # 将所有重复的ensemble预测结果转化为tensor
            temporal_means = torch.stack(temporal_means, dim=0)  # [n_repeats, bsz, 1]
            spatial_means = torch.stack(spatial_means, dim=0)  # [n_repeats, bsz, dim]

            # 计算多次ensemble间的平均值和方差
            multi_ensemble_temporal_mean = temporal_means.mean(dim=0)  # [bsz, 1]
            multi_ensemble_spatial_mean = spatial_means.mean(dim=0)  # [bsz, dim]

            # 计算多次ensemble之间的方差 (这是我们的不稳定性指标)
            temporal_var_between_ensembles = ((temporal_means - multi_ensemble_temporal_mean)**2).mean(dim=0).squeeze()  # [bsz]
            spatial_var_between_ensembles = ((spatial_means - multi_ensemble_spatial_mean)**2).mean(dim=[0, 2])  # [bsz]

            # 将方差反归一化到原始尺度
            temporal_var_between_ensembles = temporal_var_between_ensembles * ((MAX[1] - MIN[1])**2)
            spatial_var_between_ensembles = spatial_var_between_ensembles * (torch.mean((torch.tensor(MAX[2:]) - torch.tensor(MIN[2:]))**2))

            # 计算最终预测与真实值之间的误差
            real_time_gt = (event_time_non_mask[:, 0, :].detach().cpu()) * (MAX[1] - MIN[1]) + MIN[1]
            gen_temporal = multi_ensemble_temporal_mean * (MAX[1] - MIN[1]) + MIN[1]
            ...
            gen_spatial = multi_ensemble_spatial_mean * (torch.tensor([MAX[2:]]) - torch.tensor([MIN[2:]])) + torch.tensor([MIN[2:]])
            spatial_mae = torch.sqrt(torch.sum((real_loc_gt - gen_spatial)**2, dim=-1))  # [bsz]

            # 保存每个样本的方差和MAE
            all_temporal_var_between_ensembles.extend(temporal_var_between_ensembles.tolist())
            all_temporal_mae.extend(temporal_mae.tolist())
            all_spatial_var_between_ensembles.extend(spatial_var_between_ensembles.tolist())
            all_spatial_mae.extend(spatial_mae.tolist())

            total_num += batch_size

    print(f"Processed {total_num} test samples with {opt.n_repeats} ensemble repetitions each")

    # 计算相关系数
    temp_corr, temp_p = pearsonr(all_temporal_var_between_ensembles, all_temporal_mae)
    spat_corr, spat_p = pearsonr(all_spatial_var_between_ensembles, all_spatial_mae)

    print('=' * 40)
    print(f'MULTI-ENSEMBLE VARIANCE-ERROR CORRELATION ANALYSIS (n_repeats={opt.n_repeats}):')
    print('=' * 40)
    print(f'Temporal Between-Ensemble-Variance to MAE Correlation: {temp_corr:.4f} (p-value: {temp_p:.4e})')
    print(f'Spatial Between-Ensemble-Variance to MAE Correlation: {spat_corr:.4f} (p-value: {spat_p:.4e})')
    ...
    # 归一化数据以便更好比较
    temp_var_norm = min_max_normalize(all_temporal_var_between_ensembles)
    temp_mae_norm = min_max_normalize(all_temporal_mae)
    spat_var_norm = min_max_normalize(all_spatial_var_between_ensembles)
    ...
    print('归一化后的相关系数:')
    print(f'Temporal Between-Ensemble-Variance to MAE Correlation (Normalized): {temp_corr_norm:.4f}')
    print(f'Spatial Between-Ensemble-Variance to MAE Correlation (Normalized): {spat_corr_norm:.4f}')
    ...
    plt.plot(temp_var_sorted[::sample_rate], label='Multi-Ensemble Temporal Variance (Norm)', color='blue', alpha=0.7)
    plt.plot(temp_mae_sorted[::sample_rate], label='Temporal MAE (Norm)', color='red', alpha=0.7)
    plt.title(f'Temporal: Multi-Ensemble Variance vs MAE (Corr: {temp_corr_norm:.4f})')
    ...
    plt.plot(spat_var_sorted[::sample_rate], label='Multi-Ensemble Spatial Variance (Norm)', color='blue', alpha=0.7)
    plt.plot(spat_mae_sorted[::sample_rate], label='Spatial MAE (Norm)', color='red', alpha=0.7)
    plt.title(f'Spatial: Multi-Ensemble Variance vs MAE (Corr: {spat_corr_norm:.4f})')
    plt.xlabel('Sorted Sample Index')
    plt.ylabel('Normalized Value [0,1]')
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.7)
    ...
    plt.xlabel('Normalized Multi-Ensemble Variance')
    ...
    fig_path = f'multi_ensemble_var_error_corr_{opt.dataset}_{filename_without_ext}_r{opt.n_repeats}_{timestamp}.png'
    plt.savefig(fig_path)
    print(f'Multi-Ensemble figure saved to: {fig_path}')

    # 保存归一化的数据
    data_to_save = {
        # 原始数据
        'temporal_multi_ensemble_variance': all_temporal_var_between_ensembles,
        'temporal_mae': all_temporal_mae,
        'spatial_multi_ensemble_variance': all_spatial_var_between_ensembles,
    ...
    data_path = f'multi_ensemble_var_error_data_{opt.dataset}_{filename_without_ext}_r{opt.n_repeats}_{timestamp}.json'
